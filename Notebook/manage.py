...
def login():
    ...
    try:
        ...
        # 从表单中获取字段
            ...
            if form.Valid_user(username, password_candidate):
                # 写入session
...
def register():
    if "logged_in" in session:  # 如果已经登录，直接退出
        session.clear()
    ...
    try:
        if request.method == 'POST' and form.validate():  # 如果提交表单，并字段验证通过
            ...
            if form.Valid_register(username,password_candidate,password_confirm,email):
                db = MysqlUtil()
                password_candidate = sha256_crypt.hash(password_candidate)
                sql = f"insert into users(username,password,email)" \
                      f"Values('{username}','{password_candidate}','{email}')"
                print(sql)
                db.insert(sql)
                flash("注册成功，请登录",'info')
                return redirect(url_for('login'))
        ...
        print(e)
        flash("Some error happend","danger")
    return render_template('register.html', form=form)
